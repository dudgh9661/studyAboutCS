...
## 주소창에 도메인을 검색했을 때 일어나는 과정
1. 주소창에 도메인 주소를 입력한다.
2. DNS 서버를 통해 해당 도메인 주소와 매핑되는 목적지 IP(203.42.111.65)를 얻는다.
3. X.25 프로토콜을 이용해 인터넷 망에 접속한다.
4. IP(Internet Protocol)를 이용해 목적지 IP의 네트워크(203.42.111.0)까지 패킷을 전송한다.
  > 연결이 되어있지 않은 상태에서 연결을 하므로 IP는 '비연결형 프로토콜'이다.
5. 실제 목적지IP에 해당하는 Server까지는 다음과 같은 절차를 거친다.
6. ARP(논리적 주소(IP)로 물리적 주소(MAC주소를 찾아주는 프로토콜)를 이용해 네트워크 입구에서 '203.42.111.65에 해당되는 Server야 너의 LAN카드 번호( == Mac주소(Media Access Control) )를 알려줘'라고 broadcasting을 한다.
 > RARP는 물리적 주소 -> 논리적 주소로 변환해준다.
8. 해당 Server가 응답하면, 그 LAN카드 번호로 해당 Server에 접속한다.
9. 목적지 Server에 접속한 후, TCP를 이용해 목적지 IP의 port번호와 매핑되는 프로그램을 찾아낸다.
 > TCP는 이미 연결이 된 상태에서 진행이 되므로 '연결형 프로토콜'이라고 한다.
10. 위 방향의 역순으로 클라이언트에게 데이터를 반환한다.
11. 웹 브라우저는 반환받은 데이터를 해석해 우리에게 보여준다.
